# -*- coding: utf-8 -*-
"""bookworms_model.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/gist/EkaterinaSolovyova/cbf6b7fd9df819f7a2490563f0d36207/bookworms_model.ipynb
"""
...
# Загружаем рейтинг из предобработанного файла
print('Loading ratings')
with open('features4.csv', 'r') as ratings_file:
...
#новые индикаторы
readerIDs = defaultdict(lambda: len(readerIDs))
bookIDs = defaultdict(lambda: len(bookIDs))

for row in raw_ratings:
    row[0] = readerIDs[int(row[0])]
    row[1] = bookIDs[int(row[1])]
    row[2] = int(row[2])  
n_users = len(readerIDs)
n_items = len(bookIDs)
...
# To improve the recommendations, lets read in the book rubrics
print('Loading books metadata')
with open('books.csv', 'r') as books_file:
    books_file_reader = csv.reader(books_file)
    raw_book_metadata = list(book_file_reader)
    raw_book_metadata_header = raw_book_metadata.pop(0)

# Map the IDs to our internal IDs and keep track of the rubrics and titles
book_rubrics_by_internal_id = {}
book_titles_by_internal_id = {}
for row in raw_book_metadata:
    row[0] = book_to_internal_item_ids[int(row[0])]  # Map to IDs
    row[2] = row[2].split('|')  # Split up the rubrics
    book_rubrics_by_internal_id[row[0]] = row[2]
    book_titles_by_internal_id[row[0]] = row[1]

# Look at an example book metadata row
#print("Raw metadata example:\n{}\n{}".format(raw_book_metadata_header,
                                             raw_book_metadata[0]))

# Build a list of rubrics where the index is the internal book ID and
# the value is a list of [Genre, Genre, ...]
book_rubrics = [movie_genres_by_internal_id[internal_id]
                for internal_id in range(n_items)]

# Transform the rubrics into binarized labels using scikit's MultiLabelBinarizer
book_genre_features = MultiLabelBinarizer().fit_transform(movie_rubrics)
n_rubrics = book_genre_features.shape[1]
print("Binarized rubrics example for book {}:\n{}".format(movie_titles_by_internal_id[0],
                                                          book_genre_features[0]))

# Coerce the book genre features to a sparse matrix, which TensorRec expects
book_genre_features = sparse.coo_matrix(book_genre_features)

# Fit a content-based model using the rubrics as item features
print("Training content-based recommender")
content_model = tensorrec.TensorRec(
    n_components=n_rubrics,
...
content_model.fit(interactions=sparse_train_ratings_4plus,
                  user_features=user_indicator_features,
                  item_features=book_genre_features,
                  n_sampled_items=int(n_items * .01))
...
# Try concatenating the rubrics on to the indicator features for a hybrid recommender system
full_item_features = sparse.hstack([item_indicator_features, book_genre_features])
...
# Print out books that User #432 has liked
print("User 432 liked:")
for m in sparse_train_ratings_4plus[432].indices:
    print(book_titles_by_internal_id[m])

# Pull user 432's features out of the user features matrix and predict mbook ranks for just that user
...
print("User 432's held-out books:")
